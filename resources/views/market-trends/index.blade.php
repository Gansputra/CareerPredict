...
@section('content')
    <div class="space-y-8">
        ...
        <div data-aos="fade-up">
            <div
                class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 text-indigo-400 text-xs font-bold tracking-widest uppercase mb-3 border border-indigo-500/20">
                <i class="fas fa-chart-mixed"></i>
                Analisis Real-time
            </div>
            <h1 class="text-3xl font-extrabold text-white mb-1">
                Tren <span class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-blue-400">Pasar
                    Kerja</span>
            </h1>
            <p class="text-slate-400 text-sm max-w-2xl">
                Analisis mendalam dari <span class="text-white font-semibold">{{ number_format($totalJobs) }} lowongan
                    aktif</span> | temukan tren, skill yang dicari, dan peluang terbaik di pasar kerja saat ini.
            </p>
        </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                <div class="w-2.5 h-2.5 rounded-full"
                                    style="background: {{ $typeColors[$i % count($typeColors)] }}"></div>
                                <span class="text-xs text-slate-300">{{ $t['type'] }}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <div class="w-16 h-1.5 bg-slate-800 rounded-full overflow-hidden">
                                    <div class="h-full rounded-full"
                                        style="width:{{ $t['percent'] }}%; background: {{ $typeColors[$i % count($typeColors)] }}">
                                    </div>
                                </div>
        ...
        <div class="glass-dark p-6 lg:p-8" data-aos="fade-up" data-aos-delay="100">
            <div class="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p class="text-xs text-slate-500">Diekstrak dari {{ number_format($totalJobs) }} deskripsi & persyaratan
                        lowongan</p>
                </div>
                <div class="w-9 h-9 bg-amber-500/10 rounded-xl flex items-center justify-center">
            ...
            @else
                <div class="text-center py-8 text-slate-600"><i class="fas fa-chart-bar text-2xl mb-2"></i>
                    <p class="text-sm">Belum ada data skill.</p>
                </div>
            @endif
        </div>
                ...
                <div class="space-y-3">
                    @foreach($topCities as $city => $count)
                        ...
                        <div class="flex items-center gap-3">
                            <span
                                class="text-sm font-black {{ $medalColors[$loop->index] ?? 'text-slate-600' }} w-5 text-right shrink-0">
                                @if($loop->index < 3)
            ...
            <div class="glass-dark p-6 lg:p-8" data-aos="fade-up" data-aos-delay="100">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($topCompanies as $company => $count)
                        ...
                        @endphp
                        <div
                            class="bg-gradient-to-br {{ $grad }} border rounded-2xl p-4 flex items-center gap-4 hover:-translate-y-1 transition-transform duration-300">
                            <div
                                class="w-12 h-12 rounded-xl bg-slate-800 flex items-center justify-center text-white font-extrabold text-sm shrink-0">
                                {{ $initials }}
                            </div>
                            <div class="min-w-0">
