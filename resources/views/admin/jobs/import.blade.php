...
<div class="max-w-5xl mx-auto space-y-8" x-data="{ activeTab: 'api' }">
    ...
    <div class="flex justify-between items-start animate-fade-in">
        <div>
            ...
            <h1 class="text-3xl font-extrabold text-white mb-1">Job <span class="text-gradient">Importer</span></h1>
            <p class="text-slate-400 text-sm">Choose your data source: fetch from public APIs or upload a CSV dataset.</p>
        </div>
        <form action="{{ route('admin.jobs.clear') }}" method="POST" onsubmit="return confirm('⚠️ Are you sure you want to delete ALL jobs? This cannot be undone.');">
            @csrf
    ...
    <div class="grid grid-cols-3 gap-4">
        <div class="glass-dark p-5 text-center animate-fade-in">
            ...
            <p class="text-[10px] uppercase tracking-widest text-slate-500 mt-1">Total Jobs</p>
        </div>
        <div class="glass-dark p-5 text-center animate-fade-in" style="animation-delay: 60ms">
            ...
            <p class="text-[10px] uppercase tracking-widest text-slate-500 mt-1">Active</p>
        </div>
        <div class="glass-dark p-5 text-center animate-fade-in" style="animation-delay: 120ms">
            ...
            <p class="text-[10px] uppercase tracking-widest text-slate-500 mt-1">Categories</p>
        </div>
    </div>
    ...
    <div class="flex gap-2 bg-slate-900 p-1.5 rounded-2xl animate-fade-in" style="animation-delay: 200ms">
        <button @click="activeTab = 'api'" class="flex-1 py-3 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2"
            ...
            <i class="fas fa-globe"></i> Fetch from API
        </button>
        <button @click="activeTab = 'csv'" class="flex-1 py-3 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2"
            ...
            <i class="fas fa-file-csv"></i> Upload CSV Dataset
        </button>
    </div>
    ...
    <div x-show="activeTab === 'api'" x-transition class="glass p-8 animate-fade-in">
        <div class="flex items-center gap-4 mb-8 pb-6 border-b border-slate-800">
            ...
            <div>
                <h2 class="text-xl font-bold text-white">Fetch from Public APIs</h2>
                <p class="text-slate-400 text-xs">Pull real job listings from Remotive & Arbeitnow (no API key needed)</p>
            </div>
        </div>
        ...
        <form action="{{ route('admin.jobs.import.api') }}" method="POST" x-data="{ loading: false }" @submit="loading = true">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                {{-- Source --}}
                <div>
                    <label class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 block">Source</label>
                    <select name="source" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all">
                        ...
                        <option value="arbeitnow">🔵 Arbeitnow (EU Jobs)</option>
                    </select>
                </div>

                {{-- Limit --}}
                <div>
                    <label class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 block">Max Jobs</label>
            ...
            <div class="bg-slate-900/50 p-5 rounded-xl border border-slate-800 mb-8">
                <h4 class="text-xs font-bold text-white uppercase tracking-widest mb-3">Available APIs</h4>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-[11px] text-slate-500">Remote tech jobs worldwide, includes salary data</p>
                        </div>
                    </div>
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-[11px] text-slate-500">EU-focused jobs with tags for skills extraction</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <button type="submit" class="w-full btn-premium py-4 flex items-center justify-center gap-2" :disabled="loading">
                <template x-if="!loading">
                    <span><i class="fas fa-download mr-2"></i> Fetch Jobs from API</span>
                </template>
                <template x-if="loading">
                    <span><i class="fas fa-spinner fa-spin mr-2"></i> Fetching... this may take a moment</span>
                </template>
            </button>
        </form>
    </div>
    ...
    <div x-show="activeTab === 'csv'" x-transition class="glass p-8 animate-fade-in">
        <div class="flex items-center gap-4 mb-8 pb-6 border-b border-slate-800">
            ...
            <div>
                <h2 class="text-xl font-bold text-white">Upload CSV Dataset</h2>
                <p class="text-slate-400 text-xs">Import jobs from Kaggle datasets or custom CSV files</p>
            </div>
        </div>
            ...
            <div class="space-y-8">
                <div class="border-2 border-dashed border-slate-700 rounded-3xl p-12 text-center hover:border-emerald-500 transition-all group relative" x-data="{ fileName: '' }">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="text-white font-medium" x-text="fileName || 'Click to upload or drag and drop'"></div>
                        <p class="text-xs text-slate-500">CSV files only (Max 10MB)</p>
                    </div>
                </div>

                {{-- Column Mapping Guide --}}
                <div class="bg-slate-900/50 p-6 rounded-2xl border border-slate-800">
                    <h4 class="text-sm font-bold text-white mb-4 uppercase tracking-widest">Flexible Column Mapping</h4>
                    <p class="text-xs text-slate-500 mb-4">The importer auto-detects these column names (case-insensitive):</p>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <div class="flex items-center gap-2 text-xs">
                            <span class="w-2 h-2 bg-blue-400 rounded-full shrink-0"></span>
                            <span class="text-white font-bold">Salary:</span>
                        ...
                        <div class="flex items-center gap-2 text-xs">
                            <span class="w-2 h-2 bg-amber-400 rounded-full shrink-0"></span>
                            <span class="text-white font-bold">Skills:</span>
                    ...
                    <div class="mt-5 pt-4 border-t border-slate-800">
                        <p class="text-[10px] text-slate-600 mb-1">Example CSV header:</p>
